...
- **`RegisterButton.tsx`** is the only CTA (used by Navbar + Hero). It navigates to `/portal` — the self-guarding Team Portal (`src/portal/PortalPage.tsx`): Google sign-in → Firestore team lookup → registration form or team portal. Firebase config comes from `VITE_FIREBASE_*` env vars (`.env.example`); the `/portal` route is lazy-loaded so the marketing site never pulls in Firebase.
